...
class Car:
    # list of available cars, take random every time
    car_sprites = ("Audi", "Black_viper", "Orange", "Taxi")
    ...
    def __init__(self, speed=5):
        ...
        self.last_time_change_angle = 0
        self.change_angle = 0
        self.angle = 0
        self.speed = speed

        self.radar_degrees = [-90, -45,  -6, 6, 45, 90]
        self.radars = []
    ...
    def get_data(self):
        radars = self.radars
        data = list(np.zeros(len(self.radar_degrees)))

        for i, r in enumerate(radars):
            if i not in [int(len(data)/2-1), int(len(data)/2)]:
                data[i] = int(r[1] / 30)
            else:
                data[i] = int(r[1])

        new_data = data[:int(len(data)/2-1)] + [data[int(len(data)/2)] - data[int(len(data)/2-1)]] + data[int(len(data)/2+1):]
        return new_data

    def get_reward(self):
        return self.distance / 3 - self.total_rotate_angle

    def set_angle(self, change_angle):
        if change_angle != 0:
            self.last_time_change_angle = self.time_spent
        self.last_change_angle = self.change_angle
    ...
    def update(self, road):
        ...
        if self.change_angle * self.last_change_angle < 0 and self.time_spent - self.last_time_change_angle < 8:
            self.total_rotate_angle += abs(self.last_change_angle)
...
def run_generation(genomes, config):
    ...
    road = pygame.image.load('sprites/road_v2.png')
    ...
    while True:
        ...
        for i, car in enumerate(cars):
            ...
            i = output[:-1].index(max(output[:-1]))

            # this will interpret net output to car action
            # change to not constant 5 degrees, but in proportion to network output
            if i == 0:
                car.set_angle(change_angle=output[i] * 5)
            elif i == 1:
                car.set_angle(change_angle=0)
            elif i == 2:
                car.set_angle(change_angle=- output[i] * 5)

            # car speed is needed for next experiment
            car.speed = max(3, car.speed + output[-1])
            car.speed = 5
        ...
        label = font.render("Cars leave: " + str(cars_left), True, (51, 59, 70))
        label_rect = label.get_rect()
